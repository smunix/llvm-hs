      ...
      let
        ...
        mkOverlay = ghc: final: _:
          ...
          with haskell.packages."ghc${ghc}".extend (final: _: rec { }); {
            "${ghc}" = rec {
              ...
              llvm-hs-pure = overrideCabal (disableLibraryProfiling (doJailbreak
              ...
              llvm-hs = addBuildDepends (overrideCabal (disableLibraryProfiling
                (doJailbreak (callCabal2nix "llvm-hs"
                  (with nf.lib; filter { root = ./llvm-hs; }) {
                    inherit llvm-config llvm-hs-pure;
                  }))) (o: { version = "${o.version}-${version ghc}"; }))
                [ llvm-config ];
        ...
        eachGHC = ghcs:
          ...
          in with pkgs; rec {
            ...
            #   shellFor {
            #     packages = _: [
            #       packages."ghc${head ghcs}/llvm-config"
